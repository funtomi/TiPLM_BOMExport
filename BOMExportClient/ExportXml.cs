...
namespace BOMExportClient {
   public class ExportXml {
       private DataSet ds;
        ...
        public ExportXml(DataSet ds, Guid eventOid, Guid userOid) {
            //
            // TODO: 在此处添加构造函数逻辑
            //
            this.ds = ds;
        ...
        public static void ExportToXml(DataSet ds, Guid eventOid, Guid userOid) {
            ExportXml exp = new ExportXml(ds, eventOid, userOid);
            string xlsPath = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            string noFindXml = "";
            int t = 0;
            for (int i = 0; i < ds.Tables.Count; i++) {
                string tbName = ds.Tables[i].TableName;
                string xlsFile = Path.Combine(xlsPath, tbName + ".xml");
            ...
            for (int i = 0; i < ds.Tables.Count; i++) {
                ...
                if (File.Exists(xlsFile)) {
                    ...
                    ds.AcceptChanges();
                    i--;
                    exp.DoExport(tb, xlsFile, newFile);
                }
            }
        }

        private void DoExport(DataTable tb, string xlsFile, string newFile) {
